...
{
    public class PropertyOrchestratorService : IPropertyOrchestratorService
    {
        private readonly IPropertyRepository _propertyRepository;
        private readonly ILogger<PropertyOrchestratorService> _logger;

        public PropertyOrchestratorService(
            IPropertyRepository propertyRepository,
            ILogger<PropertyOrchestratorService> logger)
        {
            ...
            string baseUrl)
        {
            ...
            if (string.IsNullOrWhiteSpace(fullAddress) && entity.Location != null)
            {
                var parts = new[]
                {
        ...
        public async Task<int> CreateFullPropertyAsync(CreateFullPropertyDTO dto)
        {
            ...
            {
                var validIds =
                    await _propertyRepository.GetValidServiceIdsAsync(dto.Services);

                var invalid =
                    dto.Services.FirstOrDefault(id => !validIds.Contains(id));
            ...
            if (dto.Images != null)
            {
                ...
                {
                    long size =
                        (dto.ImageSizes != null && i < dto.ImageSizes.Count)
                            ? dto.ImageSizes[i]
                            : 0;
                    ...
                    propertyEntity.Images.Add(new PropertyImageEntity
                    {
        ...
        public async Task<bool> UpdateFullPropertyAsync(UpdateFullPropertyDTO dto)
        {
            _logger.LogInformation(
            ...
            {
                updated =
                    await _propertyRepository.UpdatePropertyCoreAsync(command);
            }
            catch (Exception ex)
            {
                _logger.LogError(
        ...
        public async Task<PagedResult<PropertyListItemDto>> GetPropertiesPagedAsync(
            PropertyListQuery query,string baseUrl)
        {
        ...
        // ================= Helper ================= //
        private static bool IsValidImage(string imagePath,long fileSizeBytes)
        {
            if (string.IsNullOrWhiteSpace(imagePath))
                return false;

            var ext =
                Path.GetExtension(imagePath.Split('?')[0]).ToLower();

            if (string.IsNullOrEmpty(ext))
            {
                ext =
                    Path.GetExtension(
                        imagePath.Split('/').Last()).ToLower();
            }
            ...
            if (fileSizeBytes > 0 && fileSizeBytes > MAX)
                return false;

            return true;
        }

        // ================= Legacy composition — لسه شغالة، بتخدم SearchByUniversity* بس ================= //
        public static object BuildUniversityPropertyResponse(clsUniversities.PropertyWithDistanceDTO r,string baseUrl)
        {
            var video =
                clsPropertyVideo.GetVideoByPropertyID(
            ...
            return new
            {
                ...
                images = clsPropertyImages
                    .GetImagesByPropertyID(r.Property.PropertyID)
                    .Select(img => new
                    {
                        img.ImageId,
                        imageUrl = $"{baseUrl}/{img.ImagePath}"
                ...
                services = clsPropertyServices
                    .GetServicesByPropertyID(r.Property.PropertyID)
                    .Select(s => new
                    {
                        s.ServiceId,
                        s.Name,
